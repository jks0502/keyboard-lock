...
Run `KeyboardLock.exe`. Press **Ctrl+Alt+K** to lock or unlock keyboard input. The tray icon also provides lock, unlock, and exit actions.
...
- `KeyboardLock.cs` — standalone WinForms source.
- `KeyboardLock.exe` — compiled Windows executable.
- `KeyboardLock.ps1` — earlier PowerShell implementation.
- `Start-KeyboardLock.cmd` — PowerShell launcher for the earlier implementation.
